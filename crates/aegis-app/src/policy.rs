...
impl AppPolicies {
    pub fn from_config(cfg: &Config) -> Result<Self, AppError> {
        ...
        let auth = AuthPolicy {
            ...
            session_idle_timeout: Duration::minutes(session.idle_timeout_minutes as i64),
            allow_unverified_login: !cfg.email.enabled,
            password_policy: PasswordPolicy {
                min_length: cfg.credentials.password.policy.min_length,
                require_uppercase: cfg.credentials.password.policy.require_uppercase,
                require_lowercase: cfg.credentials.password.policy.require_lowercase,
                require_digit: cfg.credentials.password.policy.require_digit,
                require_symbol: cfg.credentials.password.policy.require_symbol,
                disallow_common: cfg.credentials.password.policy.disallow_common,
                disallow_email_substring: true,
            },
            bearer_token_length: session.bearer.opaque_token_length as usize,
            bearer_default_ttl: Duration::minutes(session.bearer.default_ttl_minutes as i64),
            refresh_token_enabled: session.bearer.refresh_token_enabled,
            refresh_token_ttl: Duration::days(session.bearer.refresh_token_ttl_days as i64),
            revoke_all_sessions_on_password_reset: true,
        };

        let email = EmailPolicy {
            enabled: cfg.email.enabled,
            verification_token_ttl: Duration::hours(cfg.email.verification_token_ttl_hours as i64),
        ...
        let compliance = CompliancePolicy {
            ...
            deleted_user_anonymize_after: Duration::days(
                cfg.compliance.data_retention.deleted_user_anonymize_after_days as i64,
            ),
            anonymize_email_pattern: cfg.compliance.deletion.anonymize_email_pattern.clone(),
        };

        let totp = TotpPolicy {
            issuer: {
                let s = &cfg.credentials.mfa.totp.issuer;
                if s.is_empty() { "Aegis".to_owned() } else { s.clone() }
            },
            ...
            algorithm: match cfg.credentials.mfa.totp.algorithm {
                aegis_config::TotpAlgorithm::SHA1 => aegis_core::TotpAlgorithm::Sha1,
                aegis_config::TotpAlgorithm::SHA256 => aegis_core::TotpAlgorithm::Sha256,
                aegis_config::TotpAlgorithm::SHA512 => aegis_core::TotpAlgorithm::Sha512,
            },
