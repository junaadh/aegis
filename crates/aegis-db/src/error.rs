    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidTokenHashLength { expected, actual } => {
                write!(
                    f,
                    "invalid token hash length: expected {expected}, got {actual}"
                )
            }
            ...
            Self::InvalidGuestStatus(s) => {
                write!(f, "invalid guest status: {s}")
            }
            Self::InvalidTotpAlgorithm(s) => {
                write!(f, "invalid totp algorithm: {s}")
            }
