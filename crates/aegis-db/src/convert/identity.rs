use aegis_core::{
    DisplayName, EmailAddress, Guest, GuestId, GuestStatus, Metadata, Session,
    SessionId, SessionIdentity, User, UserId, UserStatus,
};
...
impl TryFrom<UserRow> for User {
    ...
    fn try_from(row: UserRow) -> Result<Self, Self::Error> {
        ...
        let email = EmailAddress::parse(&row.email).map_err(|_| {
            ConversionError::InvalidStatus(format!(
                "invalid email: {}",
                row.email
            ))
        })?;
        ...
        let display_name =
            DisplayName::parse(&row.display_name).map_err(|_| {
                ConversionError::InvalidStatus(format!(
        ...
        let mut builder =
            User::builder(UserId::from_uuid(row.id), email, display_name)
                .status(status)
                .metadata(metadata)
        ...
        builder.build().map_err(|_| {
            ConversionError::InvalidStatus("user invariant violated".to_owned())
        })
    }
}
...
impl TryFrom<GuestRow> for Guest {
    ...
    fn try_from(row: GuestRow) -> Result<Self, Self::Error> {
        let email = row
            ...
            .transpose()
            .map_err(|_| {
            ConversionError::InvalidStatus("invalid guest email".to_owned())
        })?;
        ...
        let mut builder =
            Guest::builder(GuestId::from_uuid(row.id), row.expires_at)
                .metadata(metadata)
                .created_at(row.created_at)
                .updated_at(row.updated_at);
        ...
        builder.build().map_err(|_| {
            ConversionError::InvalidGuestStatus(
                "guest invariant violated".to_owned(),
            )
        })
    }
}
...
impl TryFrom<SessionRow> for Session {
    ...
    fn try_from(row: SessionRow) -> Result<Self, Self::Error> {
        ...
        let identity = match (row.user_id, row.guest_id) {
            ...
            (None, Some(gid)) => {
                SessionIdentity::Guest(GuestId::from_uuid(gid))
            }
