...
impl SmtpEmailSender {
    ...
    pub fn from_email_config(config: &EmailConfig) -> Result<Self, AppError> {
        ...
        let from = if config.from_name.trim().is_empty() {
            ...
            Mailbox::from_str(&format!(
                "{} <{}>",
                config.from_name, config.from_address
            ))
        }
        .map_err(|e| {
            AppError::Infrastructure(format!("invalid from email address: {e}"))
        })?;
    ...
    async fn send(
        ...
        let to = Mailbox::from_str(to).map_err(|e| {
            AppError::Infrastructure(format!(
                "invalid destination email address: {e}"
            ))
        })?;
        ...
        let message = Message::builder()
            ...
            .body(body.to_owned())
            .map_err(|e| {
                AppError::Infrastructure(format!("failed to build email: {e}"))
            })?;

        self.mailer.send(message).await.map_err(|e| {
            AppError::Infrastructure(format!("smtp send failed: {e}"))
        })?;
...
impl EmailSender for SmtpEmailSender {
    async fn send_verification(
        ...
        self.send(
            ...
            &format!(
                "Use this verification token to verify your account: {token}"
            ),
        )
        .await
    }
    ...
    async fn send_password_reset(
        ...
        self.send(
            ...
            &format!(
                "Use this password reset token to reset your password: {token}"
            ),
        )
        .await
    }
